...
### distribuição de base da Chen

G=function(t,eta,kappa){
  return( 1-exp(eta*(1-exp(t^kappa))) )  
}

#g=function(t,eta,kappa){
#  return(eta*kappa*t^(kappa-1)*exp((t^kappa)+eta*(1-exp(t^kappa))))
#}

g=function(t,eta,kappa){
  return(eta*kappa*t^(kappa-1)*exp(t^kappa)*exp(eta*(1-exp(t^kappa))) ) 
}
...
theta0=c(rep(.5,3),rep(-0.001,px)) ## initial values
length(theta0)
...
legend(0,0.5, expression(hat(p)[0][F]=="0,641",IC("95%")=="[0,606; 0,676]"),pch=15,col = 2,bty = "n",cex=1.8)
legend(10,0.5, expression(hat(p)[0][M]=="0,473",IC("95%")=="[0,425; 0,521]"),pch=15,col = 4,bty = "n",cex=1.8)
